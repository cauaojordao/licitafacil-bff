...
from spark_jobs.category_classifier import CategoryClassifier
from spark_jobs.summary_generator import SummaryGenerator
...
class EditalProcessor:
    ...
    def process(self, row: Row) -> dict[str, Any]:
        ...
        return {
            ...
            "situacao_compra_nome": row.situacao_compra_nome,
            "criterio_julgamento_nome": row.criterio_julgamento_nome,
            "link_sistema_origem": row.link_sistema_origem,
            "orgao_entidade": {
                "razao_social": (
                    row.orgao_entidade.razao_social if row.orgao_entidade else None
                ),
                "cnpj": row.orgao_entidade.cnpj if row.orgao_entidade else None,
            },
            "unidade_orgao": {
                "nome_unidade": (
                    row.unidade_orgao.nome_unidade if row.unidade_orgao else None
                ),
                "uf_sigla": row.unidade_orgao.uf_sigla if row.unidade_orgao else None,
                "municipio_nome": (
                    row.unidade_orgao.municipio_nome if row.unidade_orgao else None
                ),
            },
